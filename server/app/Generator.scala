...
object Generator {
  ...
  def apply(config: Config.GenMap, turns: Int, training: Boolean, autoplay: Boolean, category: String): Try[Game] = config.size match {
    case s if s < 8 ⇒ fail("Board is too small")
    case s if s % 2 != 0 ⇒ fail("Board size is odd")
    case _ ⇒ attempt(config, turns, training, autoplay, category)
  }

  private def attempt(config: Config.GenMap, turns: Int, training: Boolean, autostart: Boolean, category: String, attempts: Int = 1): Try[Game] = {
    ...
    generateSpawnPos(boardDraft) flatMap { spawnPos =>
      ...
      if (board.countMines == 0) fail("Board has no mine")
        ...
        generateGame(finalBoard, config, turns, training, spawnPos, autostart, category)
      }
    }
  } recoverWith {
    case err if attempts < maxAttempts => {
      attempt(config, turns, training, autostart, category, attempts + 1)
    }
  }

  private def generateGame(board: Board, config: Config.GenMap, turns: Int, training: Boolean, spawnPos: Pos, autostart: Boolean, category: String) = Game(
    id = RandomString(8),
    training = training,
    board = board,
    hero1 = Hero(1, "Waiting for player 1", None, None, spawnPos),
    hero2 = Hero(2, "Waiting for player 2", None, None, board mirrorX spawnPos),
    hero3 = Hero(3, "Waiting for player 3", None, None, board mirrorXY spawnPos),
    hero4 = Hero(4, "Waiting for player 4", None, None, board mirrorY spawnPos),
    spawnPos = spawnPos,
    maxTurns = turns,
  ...
  private def placeTaverns(board: Board, heroPos: Pos, config: Config.GenMap): Try[Board] = {
    ...
    def nbReachableMines(b: Board) = b.posTiles count {
      case (p, Tile.Mine(_)) => p.neighbors map b.get exists (t => t == Some(Tile.Air) || t == Some(Tile.Spikes))
      case _ => false
    }
    ...
    def sector(size: Int) = Board {
      (1 to math.pow(size, 2).toInt).toVector map { _ =>
        Random.nextInt(100) match {
          ...
          case x if x < (config.minePercent + config.wallPercent + config.spikePercent) => Tile.Spikes
          case _ => Tile.Air
        }
      }
    }
